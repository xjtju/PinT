...
/**
 *  
 *  The class is a template of pipelined Newton-Raphson method for nonlinear system 
 *  
 *  The idea is similar with DC/RIDC 
 *
 *  It is not easy to be implemented, not yet finished
 */

class PNewtonSolver : public Solver {
...
public:
    PNewtonSolver(PinT *c, Grid *g):Solver(c,g) {
        setup(); 
    }; 

    PNewtonSolver(PinT *c, Grid *g, bool isFS) : Solver(c,g,isFS) {
        setup(); 
    }

    virtual ~PNewtonSolver() {
